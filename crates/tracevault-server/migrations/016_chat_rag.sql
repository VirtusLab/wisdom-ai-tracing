-- 016_chat_rag.sql
-- Enterprise Chat RAG: pgvector extension, embedding tables, chat persistence

-- Vector extension (requires superuser or extension permission)
...
CREATE INDEX idx_chunk_embeddings_session
    ON chunk_embeddings (session_id);

-- Indexing progress tracking
CREATE TABLE chat_indexing_status (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id UUID NOT NULL REFERENCES sessions(id) ON DELETE CASCADE,
