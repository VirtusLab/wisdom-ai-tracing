...
async fn seed_overlap(pool: &sqlx::PgPool) -> (uuid::Uuid, String, uuid::Uuid, uuid::Uuid) {
    ...
    insert_ledger(pool, org_id, user_id, repo_id, "m", Some("msg_X"), 200, 20, 70, 7, 0.20).await;
...
async fn dedup_under_both_drops_overlapping_ledger_across_breakdowns(pool: sqlx::PgPool) {
    ...
    assert_eq!(a::overview_total_tokens_for_test(&pool, org_id).await, 110, "overview");
    assert_eq!(a::tokens_by_author_total_for_test(&pool, org_id, &email).await, 110, "tokens_by_author");
    assert_eq!(a::author_tokens_for_test(&pool, org_id, &email).await, 110, "authors_leaderboard");
    assert_eq!(a::model_m_for_test(&pool, org_id).await, (110, 1), "models_distribution");
    assert!((a::cost_total_for_test(&pool, org_id).await - 0.10).abs() < 1e-9, "cost_total");
    assert_eq!(a::cost_cache_read_total_for_test(&pool, org_id).await, 50, "cost_cache_read");

    // under `proxy`: session excluded, dedup OFF → the ledger row counts in full.
    set_source(&pool, org_id, "proxy").await;
    assert_eq!(a::overview_total_tokens_for_test(&pool, org_id).await, 220, "overview proxy");
    assert_eq!(a::model_m_for_test(&pool, org_id).await, (220, 0), "models proxy");

    // under `hook`: ledger excluded entirely → session value.
    set_source(&pool, org_id, "hook").await;
    assert_eq!(a::overview_total_tokens_for_test(&pool, org_id).await, 110, "overview hook");
}
...
async fn dedup_keeps_non_overlapping_and_null_id_ledger(pool: sqlx::PgPool) {
    ...
    insert_ledger(&pool, org_id, user_id, repo_id, "n", Some("msg_Y"), 40, 10, 0, 0, 0.05).await;
    ...
    assert_eq!(a::overview_total_tokens_for_test(&pool, org_id).await, 167, "both keeps non-overlap + null");

    set_source(&pool, org_id, "proxy").await;
    // proxy: all ledger rows (220 + 50 + 7), session excluded, dedup off.
    assert_eq!(a::overview_total_tokens_for_test(&pool, org_id).await, 277, "proxy counts all ledger");
}
