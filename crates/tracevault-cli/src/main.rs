...
enum Cli {
    ...
    /// Render agent-readable instructions from active policies.
    ///
    /// Fetches the active policies for the current repo and outputs Markdown
    /// instructions describing which tools the agent must call before pushing,
    /// and how validation windows work. Designed to be invoked from CLAUDE.md
    /// (or equivalent) at session start so the agent's behaviour matches the
